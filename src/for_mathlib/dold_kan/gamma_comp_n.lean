    ...
    comm := begin
      ...
      { erw ← Γ_index_set.eq_id_iff at h,
        ...
        simp only [P_infty_eq_id_on_Γ_summand_assoc,
          N₁Γ₀_map_termwise, eq_to_hom_refl, colimit.ι_desc, cofan.mk_ι_app,
...
{ hom := N₁Γ₀_iso_hom,
  ...
  hom_inv_id' := begin
    ...
    { erw ← Γ_index_set.eq_id_iff at h,
      subst h,
      simp only [P_infty_eq_id_on_Γ_summand, eq_to_hom_refl, id_comp], },
    { erw ← Γ_index_set.eq_id_iff at h,
      erw Γ_index_set.eq_id_iff' at h,
      simp only [zero_comp, P_infty_eq_zero_on_Γ_summand K h], },
  end,
  inv_hom_id' := begin
    ext K n,
    dsimp,
