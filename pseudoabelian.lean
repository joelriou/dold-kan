...
def has_binary_biproduct_of_binary_bicone_of_projector {X : C} (p : X ⟶ X)
  [h : is_projector p] [has_kernel p] [has_kernel (𝟙 X - p)] :
  has_binary_biproduct (kernel (𝟙 X - p)) (kernel p) :=
  has_binary_biproduct_of_total (binary_bicone_of_projector p)
begin
  dsimp [binary_bicone_of_projector],
  simp only [kernel.lift_ι, add_sub_cancel'_right],
end
